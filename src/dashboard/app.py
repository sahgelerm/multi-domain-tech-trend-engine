...
# для Docker
API_URL = "http://api_semiconductors:8001"
...
with tab1:
    ...
    df["trend_score"] = pd.to_numeric(df.get("trend_score", 0), errors="coerce")
    ...
    # 👉 берём последний месяц для papers и trend
    last = df.iloc[-1]

    # 👉 патенты теперь считаем суммой 
